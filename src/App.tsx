        ...
        <section className="lg:w-1/2 bg-gray-800 p-4 sm:p-6 rounded-lg shadow-xl">
          ...
          <div className="space-y-3 sm:space-y-4 text-sm sm:text-base">
            <div>
              ...
              <p id="gender-result" className="text-teal-400 text-base sm:text-lg">{analysisResults?.gender ? `${analysisResults.gender === 'male' ? 'Nam' : 'Nữ'} (${analysisResults.genderProbability}%)` : '--'}</p>
            </div>
            <div>
              <h3 className="font-medium text-gray-300">Tuổi tác (ước tính):</h3>
              <p id="age-result" className="text-teal-400 text-base sm:text-lg">{analysisResults?.age ? `${analysisResults.age} tuổi` : '--'}</p>
            </div>
            <div>
              <h3 className="font-medium text-gray-300">Cảm xúc chính:</h3>
              <p id="expression-result" className="text-teal-400 text-base sm:text-lg">{getMentalState(analysisResults?.expression)}</p>
            </div>
            <div>
              <h3 className="font-medium text-gray-300">Nhân chủng học:</h3>
              <p id="ethnicity-result" className={`${analysisResults?.ethnicity ? 'text-teal-400' : 'text-gray-500'} text-base sm:text-lg`}>{analysisResults?.ethnicity || 'Đang phân tích...'}</p>
            </div>
            <div>
              <h3 className="font-medium text-gray-300">Sức khỏe Làn da:</h3>
              <p id="skin-health-result" className={`${analysisResults?.skinHealth ? 'text-teal-400' : 'text-gray-500'} text-base sm:text-lg`}>{analysisResults?.skinHealth || 'Đang phân tích...'}</p>
            </div>
            <div>
              <h3 className="font-medium text-gray-300">Trạng thái Thể chất:</h3>
              <p id="physical-state-result" className={`${analysisResults?.physicalState ? 'text-teal-400' : 'text-gray-500'} text-base sm:text-lg`}>{analysisResults?.physicalState || 'Đang phân tích...'}</p>
            </div>
          </div>
          
          <div className="mt-6 pt-4 border-t border-gray-700">
            <p className="text-xs text-gray-500">
              <span className="font-medium">Lưu ý:</span> Các tính năng phân tích nhân chủng học, sức khỏe làn da và trạng thái thể chất sử dụng Face++ API. Kết quả chỉ mang tính tham khảo.
            </p>
          </div>
        </section>
      </main>

      <footer className="w-full max-w-3xl text-center mt-8 sm:mt-12 py-4 border-t border-gray-700">
        <p className="text-xs sm:text-sm text-gray-500">&copy; 2025 - Phát triển bởi Manus AI. Thông tin chỉ mang tính tham khảo.</p>
      </footer>
    </div>
  );
}
